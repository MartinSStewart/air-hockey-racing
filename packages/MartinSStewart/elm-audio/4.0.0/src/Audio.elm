module Audio exposing
    ( Model, Msg, AudioData, getUserModel
...
{-| Combine multiple commands into a single command. Conceptually the same as Command FrontendOnly toBackend.batch.
-}
...
{-| A command that does nothing. Conceptually the same as Command FrontendOnly toBackend.none.
-}
...
{-| Map a command from one type to another. Conceptually the same as Command FrontendOnly toBackend.map.
-}
...
-}
type alias Ports toBackend msg =
    { toJS : JE.Value -> Command FrontendOnly toBackend (Msg msg)
    , fromJS : (JD.Value -> Msg msg) -> Subscription FrontendOnly (Msg msg)
    }
...
lamderaFrontendWithAudio :
    { init : Url.Url -> Effect.Browser.Navigation.Key -> ( model, Command FrontendOnly toBackend frontendMsg, AudioCmd frontendMsg )
    , view : AudioData -> model -> Browser.Document frontendMsg
    , update : AudioData -> frontendMsg -> model -> ( model, Command FrontendOnly toBackend frontendMsg, AudioCmd frontendMsg )
    , updateFromBackend : AudioData -> toFrontend -> model -> ( model, Command FrontendOnly toBackend frontendMsg, AudioCmd frontendMsg )
    , subscriptions : AudioData -> model -> Subscription FrontendOnly frontendMsg
    , onUrlRequest : Browser.UrlRequest -> frontendMsg
    , onUrlChange : Url -> frontendMsg
    , audio : AudioData -> model -> Audio
    , audioPort : Ports toBackend frontendMsg
    }
    ->
        { init : Url.Url -> Effect.Browser.Navigation.Key -> ( Model frontendMsg model, Command FrontendOnly toBackend (Msg frontendMsg) )
        , view : Model frontendMsg model -> Browser.Document (Msg frontendMsg)
        , update : Msg frontendMsg -> Model frontendMsg model -> ( Model frontendMsg model, Command FrontendOnly toBackend (Msg frontendMsg) )
        , updateFromBackend : toFrontend -> Model frontendMsg model -> ( Model frontendMsg model, Command FrontendOnly toBackend (Msg frontendMsg) )
...
migrateModel :
    (msgOld -> msgNew)
    -> (modelOld -> ( modelNew, Command FrontendOnly toBackend msgNew ))
    -> Model msgOld modelOld
    -> ( Model msgNew modelNew, Command FrontendOnly toBackend msgNew )
...
-}
migrateMsg : (msgOld -> ( msgNew, Command FrontendOnly toBackend msgNew )) -> Msg msgOld -> ( Msg msgNew, Command FrontendOnly toBackend msgNew )
...
updateHelper :
    (JD.Value -> Command FrontendOnly toBackend (Msg userMsg))
    -> (AudioData -> userModel -> Audio)
    -> (AudioData -> userModel -> ( userModel, Command FrontendOnly toBackend userMsg, AudioCmd userMsg ))
    -> Model userMsg userModel
    -> ( Model userMsg userModel, Command FrontendOnly toBackend (Msg userMsg) )
...
initHelper :
    ...
    -> ( model, Command FrontendOnly toBackend userMsg, AudioCmd userMsg )
    -> ( Model userMsg model, Command FrontendOnly toBackend (Msg userMsg) )
...
update :
    { a
        | audioPort : Ports toBackend userMsg
        , audio : AudioData -> userModel -> Audio
        , update : AudioData -> userMsg -> userModel -> ( userModel, Command FrontendOnly toBackend userMsg, AudioCmd userMsg )
    }
    -> Msg userMsg
    -> Model userMsg userModel
    -> ( Model userMsg userModel, Command FrontendOnly toBackend (Msg userMsg) )
...
subscriptions :
    { a | subscriptions : AudioData -> userModel -> Subscription FrontendOnly userMsg, audioPort : Ports toBackend userMsg }
    -> Model userMsg userModel
...
subscriptions app (Model model) =
    Subscription.batch
        ...
        , app.audioPort.fromJS fromJSPortSub
        ]
