...
some keys may get "stuck" in the keys list. One solution to this issue is to create a port subscription to the window `blur` events and clearing the entire key list from your model:
    ...
    -- Elm
    port blurs : (() -> msg) -> Subscription FrontendOnly msg
...
-}
downs : (RawKey -> msg) -> Subscription FrontendOnly msg
downs toMsg =
    Effect.Browser.Events.onKeyDown (eventKeyDecoder |> Json.map toMsg)


{-| Subscription for key up events.
-}
ups : (RawKey -> msg) -> Subscription FrontendOnly msg
ups toMsg =
    Effect.Browser.Events.onKeyUp (eventKeyDecoder |> Json.map toMsg)
